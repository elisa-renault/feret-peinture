...
| Demandes sans suite | Validé : Christophe supprime les demandes sans suite de la boîte de réception au maximum trois ans après la dernière interaction. L’organisation pratique et la vérification de la suppression restent à mettre en place ; les sauvegardes sont traitées séparément. | Christophe |
...
| Conservation | Demandes sans suite : au maximum trois ans après la dernière interaction, validés. Définir les autres durées (contrats, logs antispam, journaux serveur, sauvegardes) et organiser la purge pour toutes les catégories. |
