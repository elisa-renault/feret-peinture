...
/** Explicit authorization for real appointment requests on the private staging site. */
function fp_quote_live_preview_enabled() {

	return 'staging' === wp_get_environment_type()
		&& fp_form_flag( 'FP_PREVIEW_FORM_ENABLED' )
		&& fp_form_flag( 'FP_CONTACT_APPROVED' )
		&& fp_form_flag( 'FP_MAIL_DELIVERY_VERIFIED' )
		&& '' !== trim( fp_mail_setting( 'FP_PRIVACY_RETENTION' ) )
		&& is_email( fp_mail_setting( 'FP_QUOTE_TO' ) )
		&& is_email( fp_mail_setting( 'FP_MAIL_FROM' ) )
		&& '' !== fp_mail_setting( 'FP_SMTP_HOST' )
		&& in_array( fp_mail_setting( 'FP_SMTP_SECURE' ), array( 'ssl', 'tls' ), true );
}

function fp_quote_preview_delivery_allowed() {
	return fp_quote_live_preview_enabled() && ! empty( $GLOBALS['fp_quote_mail_active'] );
}
...
function fp_quote_ready() {
	...
	if ( fp_is_preview() ) {
		if ( fp_quote_live_preview_enabled() ) { return true; }
...
// Real staging SMTP is limited to the explicitly enabled appointment submission.

add_filter( 'pre_wp_mail', function ( $result ) {

		if ( fp_is_preview() && ! in_array( fp_mail_setting( 'FP_SMTP_HOST' ), array( 'mailpit', 'localhost', '127.0.0.1', '::1' ), true ) && ! fp_quote_preview_delivery_allowed() ) { return false; }
...
function fp_quote_validate( $input ) {
	...
	if ( $len( $values['description'] ) < 10 ) { $errors['description'] = 'Décrivez vos travaux en quelques mots (au moins 10 caractères).'; }
	if ( $len( $values['description'] ) > 4000 ) { $errors['description'] = 'Votre description dépasse 4 000 caractères. Raccourcissez-la pour envoyer votre demande.'; }
...
function fp_quote_submit( $input, $ip ) {
	...
	$previous_mail_scope = $GLOBALS['fp_quote_mail_active'] ?? false;
	$GLOBALS['fp_quote_mail_active'] = true;
	try {
		$sent = wp_mail( fp_mail_setting( 'FP_QUOTE_TO' ), $subject, $body, $headers );
	} finally {
		$GLOBALS['fp_quote_mail_active'] = $previous_mail_scope;
	}
...
function fp_render_quote_form() {
	...
	if ( ! fp_quote_ready() ) {
		...
		echo '<div class="form-notice"><h2>Parlons de votre projet</h2><p>' . ( fp_is_preview() ? 'Le formulaire est en cours de préparation. L’envoi des demandes sera disponible à l’ouverture du site.' : 'Le formulaire est momentanément indisponible. Utilisez les coordonnées ci-dessous pour contacter Christophe Feret.' ) . '</p>';
		...
		if ( ! fp_is_preview() && is_email( fp_info( 'public_email' ) ) ) { echo '<p><a href="' . esc_url( 'mailto:' . fp_info( 'public_email' ) ) . '">' . esc_html( fp_info( 'public_email' ) ) . '</a></p>'; }
		echo '</div>';
	...
	<form class="fp-form" action="<?php echo esc_url( home_url( '/contact/' ) ); ?>" method="post" data-quote-form data-has-errors="<?php echo $errors ? '1' : '0'; ?>">
		...
		<?php if ( fp_is_preview() && ! fp_quote_live_preview_enabled() ) : ?><p class="form-note preview-notice">Formulaire de test : utilisez des coordonnées fictives. Aucun message ne sera transmis à Christophe Feret.</p><?php endif; ?>
		...
		<p class="form-note" id="contact-hint">Indiquez votre téléphone ou votre email pour que Christophe Feret puisse vous recontacter. Un seul des deux suffit.</p>
		...
		<div class="form-field"><label for="quote-description">Vos travaux en quelques mots *</label><p class="form-note" id="description-hint">Par exemple : repeindre un salon ou remplacer le sol d’une chambre. Une surface approximative suffit si vous la connaissez.</p><textarea id="quote-description" name="fp_description" rows="3" minlength="10" maxlength="4000" required aria-describedby="description-hint<?php echo isset( $errors['description'] ) ? ' error-description' : ''; ?>" <?php echo isset( $errors['description'] ) ? 'aria-invalid="true"' : ''; ?>><?php echo esc_textarea( $values['description'] ); ?></textarea><?php if ( isset( $errors['description'] ) ) : ?><p class="form-error" id="error-description"><?php echo esc_html( $errors['description'] ); ?></p><?php endif; ?></div>
        ...
        <noscript><p>Activez JavaScript pour la vérification anti-spam, ou contactez Christophe Feret par téléphone.</p></noscript>

        </div>

		<p class="form-note form-privacy">L’EURL CHRISTOPHE FERET utilise ces informations pour organiser votre rendez-vous, sur la base des mesures précontractuelles demandées. Christophe Feret traite votre demande ; Aliant peut accéder aux données pour la maintenance. Les demandes sans suite sont conservées au maximum trois ans après le dernier échange. Pour connaître vos droits d’accès, de rectification et d’effacement et les exercer, consultez la <a href="<?php echo esc_url( home_url( '/mentions-legales/#confidentialite' ) ); ?>">politique de confidentialité</a>.</p>
