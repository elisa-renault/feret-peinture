...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_enqueue_style('fp-theme', get_stylesheet_uri(), array(), $version);
    wp_enqueue_script('fp-theme', get_theme_file_uri('/assets/site.js'), array(), $version, true);
...
function fp_theme_quote_button($label = 'Demander un devis', $class = 'button--primary', $service = '') {
...
function fp_theme_project_cards($limit = 3) {
    foreach (fp_theme_projects($limit) as $project) {
        ...
        echo '<div class="project-caption"><div><p class="eyebrow">' . esc_html(get_post_meta($project->ID, 'town', true)) . '</p><h3>' . esc_html(get_the_title($project)) . '</h3></div>' . fp_theme_arrow() . '</div></a></article>';
    }
}
function fp_theme_breadcrumb($parent = '', $url = '') {
    ...
    echo '<span aria-current="page">' . esc_html(is_post_type_archive('fp_service') ? 'Prestations' : (is_post_type_archive('fp_project') ? 'Réalisations' : get_the_title())) . '</span></nav>';
}
function fp_theme_contact_band() {
    echo '<section class="contact-band"><div class="container contact-band-inner"><div><p class="eyebrow">Votre projet commence ici</p><h2>Parlons de vos travaux.</h2><p>La commune, les pièces concernées, vos envies.<br>Quelques informations pour commencer.</p></div><div class="contact-band-actions">';
...
function fp_theme_seo_data() {
    $data = array(
        'home' => array('Peintre en bâtiment à Écouen | Feret Peinture', 'Un projet de peinture à Écouen ? Présentez vos travaux et la commune du chantier à Christophe Feret pour préparer votre demande de devis.'),
        'prestations' => array('Prestations de peinture et revêtements | Feret Peinture', 'Peinture intérieure, extérieure, revêtements muraux et sols : découvrez les familles de travaux et préparez votre projet avec Christophe Feret.'),
        ...
        'entreprise' => array('Christophe Feret, peintre à Écouen | Feret Peinture', 'Christophe Feret est implanté à Écouen, dans le Val-d’Oise. Découvrez l’entreprise et contactez directement votre interlocuteur pour vos travaux.'),
        'zone-intervention' => array('Votre chantier et sa commune | Feret Peinture', 'Christophe Feret est implanté à Écouen. Précisez la commune de votre chantier pour vérifier si votre projet peut être pris en charge.'),
        'devis' => array('Demander un devis à Christophe Feret | Feret Peinture', 'Décrivez votre projet, indiquez la commune du chantier et laissez un moyen de contact pour demander un devis à Christophe Feret.'),
        ...
        'confidentialite' => array('Confidentialité | Feret Peinture', 'Informations sur l’utilisation des données communiquées dans une demande de devis et l’exercice de vos droits.'),
        'merci' => array('Votre demande | Feret Peinture', 'Suivi de votre demande de devis à Christophe Feret.'),
    );
    ...
    if (is_404()) { return array('Page introuvable | Feret Peinture', 'Retrouvez l’accueil, les prestations ou la demande de devis de Feret Peinture.'); }
