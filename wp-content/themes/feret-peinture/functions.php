...
function fp_theme_quote_button($label = 'Demander un rendez-vous', $class = 'button--primary', $service = '') {
...
function fp_theme_breadcrumb($parent = '', $url = '') {
    ...
    echo '<span aria-current="page">' . esc_html(is_post_type_archive('fp_service') ? 'Prestations' : (is_post_type_archive('fp_project') ? 'Réalisations' : (is_page('contact') ? 'Demander un rendez-vous' : get_the_title()))) . '</span></nav>';
...
function fp_theme_seo_data() {
    ...
    $data = array(
        ...
        'entreprise' => array('Christophe Feret, peintre à Écouen | Feret Peinture', 'Christophe Feret est implanté à Écouen, dans le Val-d’Oise. Découvrez l’entreprise et contactez Christophe Feret pour vos travaux.'),
        ...
        'contact' => array('Demander un rendez-vous avec Christophe Feret | Feret Peinture', 'Présentez vos travaux et la commune du chantier à Christophe Feret. Convenez d’une visite sur place avant l’établissement du devis.'),

        'mentions-legales' => array('Mentions légales et confidentialité | Feret Peinture', 'Éditeur, hébergement, données personnelles et droits des visiteurs du site Feret Peinture.'),
        ...
        'merci' => array('Votre demande | Feret Peinture', 'Suivi de votre demande de rendez-vous avec Christophe Feret.'),
